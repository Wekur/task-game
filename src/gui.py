...
class gui(ctk.CTk):
    ...
    def __init__(self):
        ...
        # UI Elements
        self.title_label = ctk.CTkLabel(self, text="❤Task RPG❤", font=("Arial", 24))
        self.title_label.pack(pady=10)

        self.level_label = ctk.CTkLabel(self, text=f"Level: {self.level}")
        self.level_label.pack()

        self.xp_label = ctk.CTkLabel(self, text=f"XP: {self.xp}/{100}")
        self.xp_label.pack()
        ...
        # Quest entry
        self.quest_entry = ctk.CTkEntry(self, placeholder_text="Add Task...")
        self.quest_entry.pack(pady=10)

        self.kind_select = ctk.CTkComboBox(self,100,20, values=["easy", "medium","more difficult","hard"], )
        self.kind_select.pack()

        self.add_button = ctk.CTkButton(self, text="add task", command=self.add_quest)
    ...
    def update_progress(self):
        self.progress.set(self.xp / 100)

    def update_labels(self):
        self.level_label.configure(text=f"Level: {self.level}")
        self.xp_label.configure(text=f"XP: {self.xp}/{100}")
        self.update_progress()
    ...
    def add_quest(self):
        ...
        t = task(quest_text, self.kind_select.get())
